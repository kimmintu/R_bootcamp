...
The purpose of this project is to analyse the hourly demand for bike sharing in New York City in 2016. In particular, we estimate the demand based on dates and weather data by different models. The best model is evaluated by comparing models on the basis of least prediction error rate.

# Methodology
The analysis bases on the bike sharing data sets for New York City in 2016 from Kaggle [^1 https://www.kaggle.com/samratp/bikeshare-analysis#NYC-CitiBike-2016.csv] and the weather data from the Weather.com API [^2 https://api.weather.com/v1/location/KLGA:9:US/observations/historical.json].

After data preparation, we provide an graphical analysis to approach the research question on the demand for shared bikes. Then, we fit a poisson model, high level polynomial linear model followed by a random forest model to deepen our analysis.  
...
ggplot(d.total, aes(x=month, y=rental_count, color=month)) +
  ...
  ggtitle("Hourly rental distribution for on months")
